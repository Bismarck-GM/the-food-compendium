...
describe('Home Page Component', () => {
  ...
  test('Calls categoriesLoadingFalse when Store.Categories has or finished fetching items.', () => {
    renderWithRedux(<App />, { route: '/the-food-compendium/', initialState: mockCategoriesPopulatedState });
    ...
    expect(title).toBeInTheDocument();
    expect(screen.getByText(/Beef/i).closest('a')).toHaveAttribute('href', '/the-food-compendium/categories/Beef');
  });
  test('Display Error when Store.Categories.error contains message.', () => {
    renderWithRedux(<App />, { route: '/the-food-compendium/', initialState: mockCategoriesWithErrorState });
    expect(fetchCategories).not.toHaveBeenCalled();
